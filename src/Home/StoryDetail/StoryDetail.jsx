...
const StoryDetail = () => {
    ...
    const [selectedPath, setSelectedPath] = useState(null); // Track selected path
    ...
    const handlePathSelection = (pathTitle) => {
        // Update selected path state to show description
        setSelectedPath(pathTitle === selectedPath ? null : pathTitle); // Toggle selection
    };
    ...
    return (
        <div className="container mx-auto px-4 py-5">
            <h1 className="text-4xl font-mono text-teal-400 text-center mt-24 mb-12">
                {story.title}
            </h1>
            <img src={story.photo} alt={story.title} className="w-[1100px] h-[600px] ml-60  mb-4" />
            <p className="text-xl mt-14 mb-4">{story.descriptionShort}</p>
            <p className="text-lg mb-4 font-semibold">Category: {story.category}</p>

            <div className="text-xl">
                {story.storyPaths.map((path, index) => (
                    <div key={index} className="mb-4">
                        <div className="flex gap-6">
                            <p className="mt-2">Option: {index + 1}</p>
                            <button
                        ...
                        {/* Display path description only when the button is clicked */}
